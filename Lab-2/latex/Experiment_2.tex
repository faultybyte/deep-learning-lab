...
\textbf{Inference:} The bar chart confirms that the Fashion-MNIST dataset is perfectly balanced, containing exactly 6,000 training samples per class across all 10 categories.
...
\textbf{Inference:} Training accuracy continually increases towards 0.94, whereas validation accuracy plateaus near 0.88 after epoch 5. This plateau indicates that the model reaches its optimal generalization capacity early, beyond which further training yields diminishing returns.
...
\textbf{Inference:} Training loss decreases smoothly toward 0.16, while validation loss bottoms out at epoch 5 and gradually rises to 0.43. This widening gap confirms the start of overfitting in later epochs.
...
\textbf{Inference:} The model shows high accuracy on distinct classes like Trousers (980) and Bags (970), but frequently confuses visually similar upper-body garments like Shirts, T-shirts, and Pullovers. This shows the limitation of 1D flattened dense layers in capturing complex, overlapping spatial textures.
...
\textbf{Inference:} CV accuracy varies significantly between 0.65 and 0.89 across trials, this means that the model is sensitive to the hyperparameter choices. Sub-optimal configurations (trials 6 and 7) dropped performance sharply, while optimal parameters consistently achieved peak accuracies near 0.89.
...
\textbf{Inference:} Automated hyperparameter optimization improved test accuracy from 0.8497 (baseline) to 0.8726. This 2.29\% performance lift proves that tuning parameters like layer width, learning rate, and batch size enhances model generalization.
...
\begin{enumerate}
    ...
    The optimizer and learning rate had the largest impact. High learning rates (e.g., 0.1) or SGD struggled to converge, whereas Adam at 0.001 consistently delivered peak CV accuracy.

    \item \textbf{Compare Grid Search and Randomized Search.} \\
    Grid Search checks every possible combination, guaranteeing the exact best setup in the grid but it takes significantly longer to finish. Randomized Search tests a random sample of combinations, finding a nearly identical top performing setup in a fraction of the time, but doesn't guarantee the best hyperparameters.
...
In this experiment, a baseline Multi Layer Perceptron was built and trained on the Fashion MNIST dataset. Hyperparameter optimization was performed to select the best hyperparameters for the model. The optimized model achieved a test accuracy of 87.26\%, demonstrating a 2.29\% accuracy boost over the baseline model.
